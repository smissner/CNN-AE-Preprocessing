...
def load_datasets(img_width=64, img_height=64):
    ...
    raw_dental = dataset_reader.read_dental()
    ...
    #                             interpolation=cv2.INTER_CUBIC)
    return mias_images, dental_images  # , dental_images
...
if __name__ == "__main__":
    ...
    if args.load:
        ...
        mias_images, dental_images = load_datasets(img_width, img_height)  # Load mias and DX datasets
        input_train, input_test = CNN_denoiser.train_test_split(mias_images, dental_images, train_split=train_split,
                                                                shuffle_test_set=shuffle_test_set, img_width=img_width,
    ...
    # input_train, input_test = CNN_denoiser.train_test_split2(dental_images, train_split=train_split, shuffle_test_set=shuffle_test_set)  # Split 1 set to train and test sets
    # input_train, input_test = train_test_split3(mias_images, dental_images, dental_images, shuffle_test_set=True)
    ...
    for i in range(len(samples)):
        samples[i][:,:,0] = eng.medfilt3(samples[i][:,:,0],np.array([3,3,1]))
        samples[i][:,:,0] = eng.ctrHarmFilt(samples[i][:,:,0])
